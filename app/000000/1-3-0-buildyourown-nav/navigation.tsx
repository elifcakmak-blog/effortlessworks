...
const Navigation = () => {
  ...
  return (
    <header className="header">
      ...
      <Link href="/000002/buildyourown" className="logo-link">
        <Image src="/nav-titles/buildyourown.svg" alt="Elif Çakmak Logo" width={300} height={300}  />
      </Link>
      <nav className="nav">
        <Link href="/" className={`Home-button ${pathname === '/' ? 'active' : ''}`}onClick={(e) => { e.preventDefault();window.location.href = "/";}}>Main Home</Link>
        <Link href="/000002/buildyourown" className={`nav-link ${pathname === '/000002/buildyourown' ? 'active' : ''}`}>Build Your Own ➡️</Link>
